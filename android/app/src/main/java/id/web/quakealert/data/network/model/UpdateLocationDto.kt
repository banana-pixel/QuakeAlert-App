 ...
 *    *only* the four documented keys — an extra field is a 400, not a warning.
 ...
 *  - [coverageRadiusKm] is the one field that does *not* follow those replace
 *    semantics: omitting it leaves the stored radius alone. A null here means
...
data class UpdateLocationRequestDto(
    ...
    /**
     * The alert radius from the Settings slider, in kilometres (50..300).
     *
     * This is what makes the radius mean anything off-device: the server chooses
     * which FCM tokens an earthquake wakes by comparing the centroid against each
     * user's stored `coverage_radius_km`. Without it every device inside the
     * dispatch cap is woken and the Haversine gate discards the notification only
     * *after* the screen has already lit up.
     */
    @SerialName("coverage_radius_km") val coverageRadiusKm: Int? = null
)

/**
 * Response of `PUT /api/v1/users/location` (HTTP 200).
 *
 * [coverageRadiusKm] is the radius *in effect* after the update — the value just
 * sent, or the one already stored when the request omitted it — so the client can
 * confirm its preference actually landed instead of assuming it did.
 */
@Serializable
data class UpdateLocationResponseDto(
    ...
    @SerialName("location_name") val locationName: String? = null,
    @SerialName("coverage_radius_km") val coverageRadiusKm: Int = 0,
